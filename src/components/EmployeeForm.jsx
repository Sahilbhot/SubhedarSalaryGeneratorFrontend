...
export default function EmployeeForm({ initial = EMPTY, onSubmit, onCancel, loading }) {
  ...
  function field(name) {
    return {
      value: form[name],
      onChange: (ev) => { setForm(f => ({ ...f, [name]: ev.target.value })); setErrors(e => ({ ...e, [name]: '' })); },
    };
  }
  ...
  return (
    <form className="emp-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="field">
        ...
        <input type="tel" placeholder="e.g. 9876543210" {...field('phone_number')} />
      </div>
      <div className="field">
      ...
      <div className="field">
        ...
        <input type="number" min="0" step="100" placeholder="e.g. 12000" {...field('salary')} />
